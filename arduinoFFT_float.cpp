...
arduinoFFT_float::arduinoFFT_float(float *vReal, float *vImag, uint16_t samples, float samplingFrequency)
{// Constructor
	this->_vReal = vReal;
	this->_vImag = vImag;
...
uint8_t arduinoFFT_float::Revision(void)
{
	return(FFT_LIB_REV);
}

void arduinoFFT_float::Compute(float *vReal, float *vImag, uint16_t samples, uint8_t dir)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	Compute(vReal, vImag, samples, Exponent(samples), dir);
}

void arduinoFFT_float::Compute(uint8_t dir)
...
void arduinoFFT_float::Compute(float *vReal, float *vImag, uint16_t samples, uint8_t power, uint8_t dir)
{	// Computes in-place complex-to-complex FFT
	// Reverse bits
	#warning("This method is deprecated and may be removed on future revisions.")
	uint16_t j = 0;
...
void arduinoFFT_float::ComplexToMagnitude(float *vReal, float *vImag, uint16_t samples)
{	// vM is half the size of vReal and vImag
	#warning("This method is deprecated and may be removed on future revisions.")
	for (uint16_t i = 0; i < samples; i++) {
...
void arduinoFFT_float::DCRemoval(float *vData, uint16_t samples)
{
	// calculate the mean of vData
	#warning("This method is deprecated and may be removed on future revisions.")
	float mean = 0;
...
void arduinoFFT_float::Windowing(uint8_t windowType, uint8_t dir)
...
float arduinoFFT_float::MajorPeak(float *vD, uint16_t samples, float samplingFrequency)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	float maxY = 0;
	uint16_t IndexOfMaxY = 0;
...
void arduinoFFT_float::MajorPeak(float *vD, uint16_t samples, float samplingFrequency, float *f, float *v)
{
	#warning("This method is deprecated and may be removed on future revisions.")
	float maxY = 0;
	uint16_t IndexOfMaxY = 0;
...
uint8_t arduinoFFT_float::Exponent(uint16_t value)
{
